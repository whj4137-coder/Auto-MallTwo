...
3. **改业务/契约/文案 → 先走 [openspec 提案](openspec/README.md)，再改 PRD/SSOT，再改代码与测试。** 偏差按 `W` 类登记 [issues](project/issues.md)。
...
`.claude/settings.json` 注册 PreToolUse 钩子 → `.claude/hooks/commit-gate.sh`：拦截 `git commit`，先跑 `npm run verify`（typecheck + SSOT 字面值校验 `scripts/check-ssot.mjs`），不过则**阻断提交**并提示修复；带 `--no-verify` 直接拦。这把 §6 的 R5 门禁从"靠自觉"变成"git 拦着"。完整 R5–R9 工程纪律见 [CONTRIBUTING.md](CONTRIBUTING.md)。
