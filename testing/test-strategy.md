...
- `npm run verify`（typecheck + check:ssot + **L1/L2，共 68**）已接入 commit-gate hook；L3（10）因需起服务+浏览器单独 `npm run test:e2e`。
- 隔离：L2 `beforeEach` 调 `store.resetAll()`；L3 每用例先点「重置数据」；ADMIN-01 改动 Admin 上下架后自带「上架」还原，避免污染共享内存态。
- 实跑（S-044，本机普通环境）：`verify` 68 + `test:e2e` 10 全绿，含 LAYOUT-01（1280×720 无溢出）与 ADMIN-01（跨端联动）；I-019 沙箱限制已解。
