...
**M-B 部分完成**：E2E 扩至 8（+A-02/C-01/D-02/SEARCH-01）；L2 加 Admin 下架·改价→前台实时（共 25 单测/集成）。**M-C 部分完成**：搜索独立页 `/search` 已落地；api-spec 增量已补。
**下一步**：①M-B 余项 门禁矩阵 REQ-024 逐格 → ②M-F 演示脚本 + 1280×720 无滚动审计 + P0/P1 清零 + 发版。
