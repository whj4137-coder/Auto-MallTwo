...
| I-030 | W | P3 | OPEN | 会员加购错误码偏离（S-044 审计发现）：`POST /cart` 对 MEMBERSHIP 返回 `2003 SCOPE_BLOCKED`，但 PRD §15.10.3 期望 `4000 VALIDATION`（仅 DISPLAY_SERVICE 应返 2003）。会员无前台加购入口，仅强制调用可触发，演示不受影响。待决策：①改代码对会员分支返 4000；或②改 PRD §15.10.3 接受统一 2003 | S-044 | - | §15.10.3 / REQ-005 |
| I-031 | W | P3 | OPEN | 会员 checkout 创建未校验 INACTIVE（S-044 审计发现）：PRD §15.10.4 要求 BUY_NOW/MEMBERSHIP 创建 checkout 时校验 `membership=INACTIVE`；实现把幂等延到支付步（`pay` 处已 ACTIVE 则不再产生新单）。最终不变量成立且有 L2 覆盖，仅校验时机与规格描述不符。待决策：①创建时前置校验对齐规格；或②改 PRD 接受"支付步幂等"口径 | S-044 | - | §15.10.4 / REQ-005 |
