...
| I-030 | W | P3 | RESOLVED | 会员加购错误码偏离（S-044 审计发现）：`POST /cart` 对 MEMBERSHIP 返回 2003，PRD §15.10.3 期望 4000。用户决策"改代码对齐 PRD"：S-046 走 [change 0023]，cart.ts 会员加购→4000、展示服务→2003，补 L2 | S-044 | S-046 | §15.10.3 / change 0023 |
| I-031 | W | P3 | RESOLVED | 会员 checkout 创建未校验 INACTIVE（S-044 审计发现）：PRD §15.10.4 要求创建时校验 `membership=INACTIVE`，实现延到支付步。用户决策"改代码对齐 PRD"：S-046 走 [change 0023]，checkout.ts 会员 checkout 创建时 ACTIVE→4000（复用 C023），B-01 幂等用例同步更新 + 补 L2 | S-044 | S-046 | §15.10.4 / change 0023 |
